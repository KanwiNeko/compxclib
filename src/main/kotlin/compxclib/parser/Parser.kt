...
class Parser(tks: ComplexList,val numbers: List<ComplexNumber>) {
    // Declarations
    ...
    private fun parseTerm(): AstMember {
        var leftHandSide = this.parseFunction()

        while (this.at() in multiplicativeOperators) {
            val op = this.at()

            val operator = if (op == Pair(Tokens.OPERATOR, "*")) Operator.TIMES
            else if (op == Pair(Tokens.OPERATOR, "/")) Operator.DIVIDE
            else Operator.MODULO

            this.eatToken(Tokens.OPERATOR)
            val rhs = this.parseFunction()
            leftHandSide = BinaryExpression(value = Operation(
                leftHandSide,
                operator,
                rhs
    ...
    private fun parseFunction(): AstMember {
        val leftHandSide = this.parseFactor()
    ...
    private fun parseFactor(): AstMember {
        ...
        return if (this.at().first == Tokens.COMPLEX_NUMBER) {
            ...
            if (this.at() == leftParenthesis) {
                ...
                val function = Dictionaries.functionMap[this.at().second]
